...
async def ingest_pdf(contents: bytes, filename: str, user_id: str = "") -> dict:
    ...
    # Attach user_id to each chunk
    for c in chunks:
        c.user_id = user_id
    chunks_with_vectors = await embed_chunks(chunks)
    await save_source(source_id=source_id, label=filename, source_type="pdf",
...
async def ingest_url(url: str, user_id: str = "") -> dict:
    source_id = str(uuid.uuid4())
    ...
    for c in chunks:
        c.user_id = user_id
    chunks_with_vectors = await embed_chunks(chunks)
    await save_source(source_id=source_id, label=label, source_type="url",
                      chunk_count=len(chunks), user_id=user_id)
    await save_chunks(chunks_with_vectors)
